...
export default function TagInPost(props: Props) {
  const [hidden, setHidden] = useState(true)
  const { seen, name, img, post, time, index } = props
            ...
            <View style={common.flexRowBet}>
              <View style={{ width: (232 / 375) * Width }}>
                <Text style={styles.text1}>
                  <Text style={styles.semiboldFont}>{name}</Text> tagged you in his post{' '}
                  <Text style={common.color1}>{post}</Text>
                </Text>
              </View>
